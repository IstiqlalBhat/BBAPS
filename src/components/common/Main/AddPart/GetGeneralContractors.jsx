...
const GetGeneralContractors = (props) => {
    ...
    const getGeneralContractors = async (evt) => {
        evt.preventDefault();
        console.log(address);
        await mainContract.methods.generalContractors(address).call()
            .then(res => {{
                    let resData = {
                        id: parseInt(res.id),
                        name: res.name,
    ...
    return (
        <div class="bg-[#2E3A41] mt-[2%] p-2 h-auto shadow-md text-center md:w-[50%] lg:w-[35%] sm:w-[60%] mx-auto" style={{border: '1px solid #5C666C'}}>
            <p class="text-lg text-white font-bold mb-2">Get General Contractors:</p>
            ...
            <p>
                <button className="w-[70%] mt-2 text-white font-medium rounded-md p-2 bg-[#32c0ca]" onClick={getGeneralContractors}>Get SubContractors</button>
            </p>
